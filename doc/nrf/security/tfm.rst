...
The ITS encryption is transparent to the user as long as the Master Key Encryption Key (MKEK) is populated before usage.

On Nordic Semiconductor devices, the hardware-accelerated AEAD scheme ChaChaPoly1305 is used with a 256 bits key.
This key is derived with a key derivation function (KDF) based on CMAC.
...
The random seed is generated once in the boot process and it stays the same until reset.
